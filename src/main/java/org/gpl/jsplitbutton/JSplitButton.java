...
/**
 * An implementation of a "split" button.The left side acts like a normal button, right side has a
 * jPopupMenu attached. <br />
 ...
 * <br />
 * <b>Known Issue:</b><br />
 * The 'button part' of the splitbutton is being drawn without the border??? and this is only
 * happening in CDE/Motif and Metal Look and Feels. GTK+ and nimbus works perfect. No Idea why? if
 * anybody could point out the mistake that'd be nice.My email naveedmurtuza[at]gmail.com<br />
 * <br />
 * P.S. The fireXXX methods has been directly plagarized from JDK source code, and yes even the
 * javadocs..;)<br />
 * <br />
...
public class JSplitButton extends JButton implements MouseMotionListener, MouseListener, ActionListener, Serializable {
    ...
    /**
     * Returns the separatorSpacing. Separator spacing is the space above and below the separator(
     * the line drawn when you hover your mouse over the split part of the button).
     * 
    ...
    /**
     * Sets the separatorSpacing.Separator spacing is the space above and below the separator( the
     ...
     * Show the dropdown menu, if attached, even if the button part is clicked.
     * 
     * @param alwaysDropDown true to show the attached dropdown even if the button part is clicked,
     *            false otherwise
     */
    ...
    /**
     * Gets the image to be drawn in the split part. If no is set, a new image is created with the
     * triangle.
     * 
     * @return image
     */
    ...
    /**
     * Removes an <code>SplitButtonActionListener</code> from the button. If the listener is the
     * currently set <code>Action</code> for the button, then the <code>Action</code> is set to
     * <code>null</code>.
     *
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == SplitButtonActionListener.class) {
                ...
                if (e == null) {
                    ...
                    e = new ActionEvent(JSplitButton.this, ActionEvent.ACTION_PERFORMED, actionCommand,
    ...
    private void fireSplitbuttonClicked(ActionEvent event) {
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == SplitButtonActionListener.class) {
                ...
                ((SplitButtonActionListener) listeners[i + 1]).splitButtonClicked(e, (JComponent) event.getSource());
            }
        }
    }
}
